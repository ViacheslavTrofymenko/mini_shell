...
/*   Updated: 2025/07/18 14:58:47 by vtrofyme         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_heredocs(t_shell *shell)
{
	...
	while (i < shell->num_cmds)
	{
		...
		{
			if (shell->cmds[i].rw_type[j] == IO_INPUT
				&& shell->cmds[i].f_mode[j] == IO_DOUBLE)
...
static void	process_single_heredoc(t_shell *shell, int i, int j)
{
	...
	if (fd < 0)
		crit_except(shell, ft_perror_custom(tmp_filename, errno));
	while (1)
	{
		line = readline("> ");
